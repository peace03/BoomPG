...
## 6.1 인스펙터에 노출되는 필드 (`[SerializeField]`)

밸런스 수치는 `ScriptableObject`에 두고 **플레이 중에 조정하며 체감**하기로 했습니다
([ADR-0005](../decisions/ADR-0005-프로젝트-코드-구조.md)). 그런데 인스펙터에 숫자만 늘어서 있으면
어떤 값이 무엇을 바꾸는지 알 수 없어 튜닝이 추측이 됩니다. 아래 셋을 의무로 합니다.

**1) `[Tooltip]` — 모든 `[SerializeField]` 필드에 한국어 설명을 단다**

설명에는 세 가지를 담습니다.

- **무엇인지** — 이 값이 가리키는 대상
- **올리면 / 내리면 어떻게 되는지** — 게임플레이에 미치는 영향
- **단위** — `m`, `m/s`, `초`, `HP`, `배율`, `비율`

```csharp
[Tooltip("로켓의 초기 비행 속도 (m/s). 높이면 예측 조준이 쉬워져 회피가 어려워진다.")]
[SerializeField] private float _rocketSpeed = 28f;
```

"로켓 속도" 처럼 필드명을 되풀이하는 툴팁은 쓰지 않습니다. **읽는 사람이 값을 바꿀지 말지
판단할 수 있어야** 합니다.

**2) `[Header]` — 논리 단위로 그룹을 나눈다**

필드가 5개를 넘으면 반드시 나눕니다. 제목은 한국어 명사구로 짧게 씁니다.
...
**3) `[Range]` — 범위가 정해진 값은 슬라이더로 만든다**

비율(0~1)이나 배율처럼 상·하한이 분명한 값은 `[Range]`를 붙입니다. 플레이 중 드래그로
조정할 수 있어 체감 비교가 빨라지고, 실수로 음수나 과도한 값을 넣는 것도 막힙니다.
...
속성 순서는 `[Header]` → `[Tooltip]` → `[Range]` → `[SerializeField]` 로 통일합니다.

> 이 규칙은 `ScriptableObject` 설정 클래스뿐 아니라 `MonoBehaviour` 의 인스펙터 노출 필드에도
> 똑같이 적용합니다. 공개 API의 `///` 주석(7장)과 별개이며, 둘 다 씁니다 —
> `///` 는 코드를 읽는 사람을 위한 것이고 `[Tooltip]` 은 인스펙터에서 값을 만지는 사람을 위한 것입니다.
...
| 2026-09-16 | 6.1 신설 — `[SerializeField]` 필드에 `[Tooltip]`·`[Header]`·`[Range]` 의무화 |
