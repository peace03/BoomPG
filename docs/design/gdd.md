...
### 23.3 넉백 중 공중 제어 규칙 (2026-09-16 결정)

> 5.1의 "공중 조작력: 지상 대비 40%"를 **넉백 상황에 한해 보완**하는 규칙입니다.
> M1 플레이 중 확인된 문제에 대한 대응이며, 다음 기획서 개정 때 본문으로 옮기는 것을 권합니다.

**문제** — 공중 조작력이 전 방향에 적용되면 **밀려나는 방향의 반대로 입력해 브레이크를 걸 수 있다.**
낙사가 주 처치 수단인 게임에서 넉백의 위협을 맞은 쪽이 스스로 무력화하는 셈이라 P1이 훼손된다.
그렇다고 공중 제어를 완전히 막으면 날아가는 동안 할 수 있는 것이 없어져 P2("아슬아슬한 복귀")의
판단 여지가 사라진다.

**결정** — 넉백 중에는 날아가는 축으로 가속·감속을 할 수 없게 하고, **그 축의 좌우로만** 움직인다.

| 상황 | 조작 가능 범위 |
|---|---|
| 지상 | 전 방향 100% |
| 공중 (넉백 없음) | 전 방향 40% — `AirControl` |
| **공중 (넉백 중)** | **날아가는 방향의 좌우로만 50%** — `KnockbackLateralControl` |
| 공중 (제트팩 추진 중) | 전 방향 4 m/s — 연료를 지불하고 사는 값어치이므로 제한하지 않는다 |

넉백 중 판정은 외부 속도의 **수평 성분이 1 m/s 이상**일 때다 (`KnockbackControlThreshold`).
그보다 느려지면 일반 공중 조작으로 돌아간다.

**의도** — 밀려나는 거리는 상대가 정하고, 착지 지점은 자신이 고른다.
P1의 위협을 유지하면서 P2의 판단 여지를 남기는 절충이다.
수치는 `SO_MovementConfig` 에서 조정할 수 있다.
...
| 2026-09-16 | 23.3 추가 — 넉백 중 공중 제어를 날아가는 방향의 좌우로만 제한 (아키텍트) |