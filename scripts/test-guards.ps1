...
$cases = @(
    @{ Guard = 'guard-planner'; Tool = 'Write'; Input = @{ file_path = 'BoomPG/Assets/Player.cs' }; Expected = 2 },
    ...
    @{ Guard = 'guard-bash'; Tool = 'Bash'; Input = @{ command = 'cat BoomPG/Assets/Player.cs > /dev/null' }; Expected = 0 },
    @{ Guard = 'guard-bash'; Tool = 'PowerShell'; Input = @{ command = 'Set-Content -LiteralPath scripts/guard-bash.ps1 -Value $c' }; Expected = 0 },
    @{ Guard = 'guard-bash'; Tool = 'Bash'; Input = @{ command = 'sed -i s/a/b/ BoomPG/Assets/Player.cs' }; Expected = 2 },
    @{ Guard = 'guard-bash'; Tool = 'Bash'; Input = @{ command = "cat > TASK_PLAN.md <<'EOF'`nOut-File BoomPG/Assets/Player.cs`nEOF" }; Expected = 0 },
    @{ Guard = 'guard-bash'; Tool = 'Bash'; Input = @{ command = 'echo x > BoomPG/Assets/Player.cs' }; Expected = 2 },
    ...
    @{ Guard = 'guard-bash'; Tool = 'Bash'; Input = @{ command = 'cp README.md BoomPG/Assets/Player.cs' }; Expected = 2 },
